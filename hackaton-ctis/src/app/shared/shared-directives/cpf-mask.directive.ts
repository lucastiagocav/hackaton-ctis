...
export class CpfMaskDirective implements ControlValueAccessor {
  ...
  onKeyup($event: any) {
    ...
    let pad = this.idMask.replace(/\D/g, '').replace(/9/g, '_');
  ...
  onBlur($event: any) {
    if ($event.target.value.length === this.idMask.length) {
      return;
    }
    this.onChange('');
    $event.target.value = '';
  ...
  aplicarMascara(valor: string): string {
    ...
    valor = '';
    for (let i = 0; i < this.idMask.length; i++) {
      if (isNaN(parseInt(this.idMask.charAt(i)))) {
        valor += this.idMask.charAt(i);
      } else {
